 ...
 * From https://github.com/javiermarinros/jquery_fm
 */
class FileManager {
    ...
    public $strings = array(
        ...
        'delete' => 'Supprimer',
        'rename' => 'Renommer',
        'confirm_delete' => 'Êtes-vous certain(e) de vouloir supprimer ce fichier ? Cette opération est irréversible !',
        'prompt_newname' => 'Saisissez le nouveau nom du fichier :',
        'drop_files' => 'Faites glisser vos fichiers ici !',
        'max_file_size' => '%size% max par fichier',
        'accept' => 'Valider',
        'cancel' => 'Anuuler',
        'start_upload' => 'Démarrer le chargement',
        'create_folder' => 'Créer un répertoire',
        'create_folder_prompt' => 'Saisissez le nom du nouveau répertoire :',
        'success' => 'Opération effectuée !',
        'error' => 'Erreur de traitement',
        'number_files' => '%count% fichiers',
        'home_folder' => 'Accueil',
        'try_again' => 'Réessayer',
    );
    ...
    private function _render_file_item(FileManagerItem $file) {
        ...
        $default_img = $this->static_url . '/img/files/unknown.png';

        if ($file->is_folder){
            $img_src = $this->static_url . "/img/files/folder.png";
						$type = 'folder';
            ...
            $img_src = $this->ajax_endpoint . (strpos($this->ajax_endpoint, '?') !== FALSE ? '&' : '?') . 'action=download&file=' . $file->name.'&folder='.$file->folder;
						$type = 'image';
				}else{
            $img_src = $this->static_url . "/img/files/{$extension}.png";
						$type = 'extension';
				}
        ob_start();
        ?>
        <div class="file" data-file="<?php echo $file->name ?>">
            <div class="image-holder">             
                ...
                <?php else: ?>
										<a href="<?php echo $this->ajax_endpoint . (strpos($this->ajax_endpoint, '?') !== FALSE ? '&' : '?') . 'action=download&file=' . $file->name.'&folder='.$file->folder.'&disp=true'; ?>" data-filetype="<?php echo $type; ?>" class="thumbnail" title="<?php echo $file->name ?>"><img src="<?php echo $img_src; ?>" class="img-rounded" onerror="this.src='<?php echo $default_img; ?>'" /></a>
                <?php endif; ?>                   
            </div>
						<script>
							jQuery(document).ready(function ($) {
								$('.thumbnail').click(function(e){
									if ($(this).data('filetype') == 'image'){
										$('#lightbox .modal-dialog').css('width', '70%');
								  	$('.modal-body').empty();
            ...
            if ($this->allow_editing) {
                ...
                <div class="file-tools">
                        ...
                        <input type="hidden" name="file" value="<?php echo $file->name ?>" />
												<button class="btn btn-primary btn-xs" type="submit" title="<?php echo $this->strings['download'] ?>"><i class="glyphicon glyphicon-download"></i></button> 
                  </form>
								<?php } ?>                    
                    <form method="POST">
                        ...
                        <input type="hidden" name="file" value="<?php echo $file->name ?>" />
                        <input type="hidden" name="folder" value="<?php echo $file->folder ?>" />
                        <button type="submit" class="btn btn-danger btn-xs delete" title="<?php echo $this->strings['delete'] ?>"><i class="glyphicon glyphicon-trash"></i></button>
                    </form>
                    <a class="btn btn-info btn-xs rename" title="<?php echo $this->strings['rename'] ?>"><i class="glyphicon glyphicon-edit"></i></a>         
                </div>
                <?php
            }
            ?>
    ...
    public function process_request($output_response = TRUE) {
        ...
        if ($output_response and !isset($_REQUEST['disp'])) {
            if ($status != 200) {
    ...
    private function _format_size($size, $kilobyte = 1024, $format = '%size% %unit%') {
        if ($size < $kilobyte) {
            ...
            $units = array('Ko', 'Mo', 'Go', 'To', 'Po', 'Eo', 'Zo', 'Yo');
